...
int main(int argc, char **argv) {
  ...
  if (argc == 3) {
    ...
    cerr<<"Missing argument try ./Eval_Seg <Im1><Im2>"<<endl;
    cerr<<"or  try ./Eval_Seg ./test/004SC3_gt.bmp ./test/seg_gmm5.jpg" <<endl;
  
    return(0);
  }
  
  
  Mat im_ref = imread(im1_str,CV_LOAD_IMAGE_GRAYSCALE);
  Mat im_seg = imread(im2_str,CV_LOAD_IMAGE_GRAYSCALE);
  
  
  
  
  float result2=F1score(im_ref,im_seg);
    cout<<"F1_score "<<result2<<endl;
    
    float result=MCC(im_ref,im_seg);
      cout<<"MCC "<<result<<endl;
  float result3=Hafiane(im_ref,im_seg);
  

  

  
  cout<<"Hafiane "<<result3<<endl;
